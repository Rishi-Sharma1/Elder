...
export default function AdminActivityMonitoring({ navigation }) {
  ...
  const types = [
    { label: "Medicine", type: "medicine", color: kineticColors.accent, icon: "💊" },
    { label: "Food", type: "food", color: kineticColors.foreground, icon: "🍽️" },
    { label: "Emergency", type: "emergency", color: kineticColors.error, icon: "🚨" },
  ];
  const statuses = [
  ...
  const roles = [
    { label: "Elders", role: "elder", color: kineticColors.mutedForeground, icon: "👴" },
    { label: "Volunteers", role: "volunteer", color: kineticColors.foreground, icon: "🤝" },
    { label: "NGOs", role: "ngo", color: kineticColors.accent, icon: "🏢" },
    { label: "Admins", role: "admin", color: kineticColors.error, icon: "🛡️" },
  ];
      ...
      <View style={[s.layout, { flexDirection: responsive.showSidebar ? "row" : "column" }]}>
        ...
        <ScrollView style={s.content} contentContainerStyle={[s.cc, { padding: responsive.contentPadding }]}
          ...
          <View style={[s.statsRow, { flexDirection: responsive.isMobile ? "column" : "row" }]}>
            <View style={[s.statCard, { borderTopColor: kineticColors.accent, borderTopWidth: 3 }]}>
            ...
            <View style={[s.statCard, { borderTopColor: kineticColors.foreground, borderTopWidth: 3 }]}>
                ...
                {["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map((d, i) => {
                  ...
                  return (
                    <View key={d} style={s.barW}>
                      <Text style={[s.barV, i===4&&{color:kineticColors.accent}]}>{Math.round(h[i]*0.8)}</Text>
                      <View style={[s.bar, {height:h[i], backgroundColor: i===4?kineticColors.accent:`${kineticColors.accent}60`}]} />
          ...
          <View style={s.tableCard}>
            ...
            {recentRequests?.length > 0 ? recentRequests.map((r, i) => (
              <View key={r._id} style={[s.tR, i%2===0&&s.tRA]}>
                ...
                <View style={s.cell}>
                  <View style={[s.typeBadge, {backgroundColor: r.type==="medicine"?`${kineticColors.accent}20`:r.type==="food"?`${kineticColors.foreground}20`:`${kineticColors.error}20`}]}>
                    <Text style={[s.typeBT, {color: r.type==="medicine"?kineticColors.accent:r.type==="food"?kineticColors.foreground:kineticColors.error}]}>
                      {r.type?.charAt(0).toUpperCase()+r.type?.slice(1)}
                    </Text>
                  </View>
                </View>
                <View style={s.cell}><Text style={s.cT}>{r.volunteer?.name||"Unassigned"}</Text></View>
                <View style={s.cell}><Text style={[s.cT, {color: r.status==="completed"?kineticColors.foreground:r.status==="assigned"?kineticColors.accent:kineticColors.mutedForeground}]}>{r.status?.charAt(0).toUpperCase()+r.status?.slice(1)}</Text></View>
                <View style={s.cell}><Text style={[s.cT, {color:kineticColors.mutedForeground}]}>{r.createdAt?new Date(r.createdAt).toLocaleDateString("en-US",{month:"short",day:"numeric"}):"—"}</Text></View>
              </View>
...
const s = StyleSheet.create({
  ...
  statCard:{flex:1,backgroundColor:kineticColors.background,padding:22,borderRadius:14,borderWidth:1,borderColor:kineticColors.border},
  statLabel:{color:kineticColors.mutedForeground,fontSize:13,fontWeight:"500",marginBottom:8},
  statBig:{fontSize:36,fontWeight:"800",color:kineticColors.foreground},
  statSub:{color:kineticColors.mutedForeground,fontSize:12,marginTop:4},
  chartsGrid:{gap:16,marginBottom:24},
  chartCard:{flex:1,backgroundColor:kineticColors.background,padding:24,borderRadius:14,borderWidth:1,borderColor:kineticColors.border},
  chartTitle:{color:kineticColors.foreground,fontSize:18,fontWeight:"700",marginBottom:4},
  chartSub:{color:kineticColors.mutedForeground,fontSize:13,marginBottom:20},
  ...
  barV:{color:kineticColors.mutedForeground,fontSize:11},
  bar:{width:"100%",minWidth:18,borderRadius:4},
  barL:{color:kineticColors.mutedForeground,fontSize:11},
  donutRow:{flexDirection:"row",justifyContent:"space-around",marginBottom:20},
  donutItem:{alignItems:"center",gap:6},
  donutCircle:{width:72,height:72,borderRadius:36,borderWidth:4,justifyContent:"center",alignItems:"center",backgroundColor:`${kineticColors.background}60`},
  donutPct:{fontSize:14,fontWeight:"800"},
  donutIcon:{fontSize:20},
  donutLabel:{color:kineticColors.mutedForeground,fontSize:12,fontWeight:"600"},
  ...
  progressL:{color:kineticColors.foreground,fontSize:13},
  progressV:{fontWeight:"700",fontSize:14},
  progressBg:{height:8,backgroundColor:`${kineticColors.border}60`,borderRadius:4,overflow:"hidden"},
  progressFill:{height:"100%",borderRadius:4},
  statusGrid:{flexDirection:"row",gap:12,flexWrap:"wrap"},
  statusCard:{flex:1,minWidth:100,backgroundColor:`${kineticColors.background}60`,padding:16,borderRadius:12,alignItems:"center",gap:8},
  ...
  statusCL:{color:kineticColors.mutedForeground,fontSize:12,fontWeight:"600"},
  statusCV:{fontSize:24,fontWeight:"800"},
  statusBarBg:{width:"100%",height:6,backgroundColor:`${kineticColors.border}40`,borderRadius:3,overflow:"hidden"},
  statusBarF:{height:"100%",borderRadius:3},
  roleGrid:{flexDirection:"row",gap:12,flexWrap:"wrap"},
  roleCard:{flex:1,minWidth:100,backgroundColor:`${kineticColors.background}60`,padding:16,borderRadius:12,alignItems:"center",gap:6},
  roleIcon:{fontSize:28},
  roleLabel:{color:kineticColors.mutedForeground,fontSize:12,fontWeight:"600"},
  roleCount:{fontSize:22,fontWeight:"800"},
  rolePBg:{width:"100%",height:6,backgroundColor:`${kineticColors.border}40`,borderRadius:3,overflow:"hidden"},
  rolePF:{height:"100%",borderRadius:3},
  rolePT:{color:kineticColors.mutedForeground,fontSize:11},
  secH:{flexDirection:"row",alignItems:"center",gap:10,marginBottom:14,marginTop:8},
  secI:{fontSize:22},
  secT:{fontSize:20,fontWeight:"700",color:kineticColors.foreground},
  tableCard:{backgroundColor:kineticColors.background,borderRadius:14,borderWidth:1,borderColor:kineticColors.border,overflow:"hidden",marginBottom:24},
  tH:{flexDirection:"row",backgroundColor:`${kineticColors.accent}15`,paddingHorizontal:16,paddingVertical:14,borderBottomWidth:1,borderColor:kineticColors.border},
  tHT:{flex:1,color:kineticColors.accent,fontWeight:"700",fontSize:12,textTransform:"uppercase",letterSpacing:0.5},
  tR:{flexDirection:"row",paddingHorizontal:16,paddingVertical:14,borderBottomWidth:1,borderColor:`${kineticColors.border}60`,alignItems:"center"},
  tRA:{backgroundColor:`${kineticColors.background}40`},
  cell:{flex:1,justifyContent:"center"},
  cT:{color:kineticColors.foreground,fontSize:13},
  ...
  emptyI:{fontSize:36},
  emptyT:{color:kineticColors.mutedForeground,fontSize:15},
});
